...
void main(int counter)
{
	...
	for (;;)
	{
		...
		case 2:  cout << "\nВведите ключ: ";
			...
			cin >> k;
			insert(key, s, &Root);
			cout <<"Количество вершин ==K: "<< counter<<endl;
			break;
		case 3:  cout << "\nВведите ключ: ";
			cin >> key;
			rc = search(Root, key);
...
void insert(int a, string s, Tree** t)  //Добавление элемента a 
{
	int counter = 0;
	...
	if (a > (*t)->key)  //дерево есть, если а больше текущего
		insert(a,s, &(*t)->Right);  //то элемент помещается вправо
	else
		insert(a,s, &(*t)->Left);   //иначе - влево
	if ((*t)->key == k) {
		counter += 1;
	}
}
